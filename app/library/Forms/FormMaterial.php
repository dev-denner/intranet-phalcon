...
class FormMaterial extends \Phalcon\Forms\Form {
    ...
    function _renderDefault($element) {
        ...
        $html .= "<label class=\"fg-label\" for=\"{$element->getName()}\">{$element->getLabel()}</label>";
        ...
        $html .= '</div>';
        if ($hasMessages) {
            $html .= "<small class=\"help-block\">{$messages[0]}</small>";
            $html .= '<span class="zmdi zmdi-close form-control-feedback"></span>';
        }
        $html .= '</div>';
        return $html;
    }
    ...
    function _renderRadio($element) {
        ...
        $divError = $hasMessages ? 'has-error' : "";
        $html = "<div class=\"toggle-switch {$divError}\" data-ts-color=\"green\">";
        //$html .= "<div class=\"radio {$divError}\">";
        $html .= $this->render($element->getName(), ['hidden' => 'hidden']);
        $html .= "<label class=\"ts-helper\" for=\"{$element->getName()}\"></label>";
        if ($hasMessages) {
            $html .= "<span class=\"help-block\">{$messages[0]}</span>";
        }
        //$html .= '</div>';
